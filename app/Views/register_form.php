...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        <form action="<?= url_to('AuthController::registerAttempt') ?>" method="post">
            ...
            <div class="mb-6">
                <label for="password_confirm" class="block text-gray-700 text-sm font-bold mb-2">Confirm
                    Password:</label>
                <input type="password" name="password_confirm" id="password_confirm"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline <?= isset($validation) && $validation->hasError('password_confirm') ? 'border-red-500' : '' ?>"
                    required>
            </div>
